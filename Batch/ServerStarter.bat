...
::cRYluBh/LU+EWAnk
::YxY4rhs+aU+IeA==
::cxY6rQJ7JhzQF1fEqQJlZkoaHErXXA==
::ZQ05rAF9IBncCkqN+0xwdVtCGUrTcj7a
...
::Zh4grVQjdCuDJFSN+kczOhpSWDgEwIY/7HSZESDHahpIJ6fVVuMrRAoVNjJm0GXgC3lhhVmoPLIajp5CXVUNMBeza28=
::YB416Ek+Zm8=
::
::
...
if "%AutoRestart%"=="开" (if "%AutoPort%"=="开" (title [%Title%]--重启[%ReTimes%]次--端口=[%Port%]--内存=[%MinRAM%/%MaxRAM%]--上次重启于[%date:~5,2%.%date:~8,2%-%time:~0,2%:%time:~3,2%:%time:~6,2%]--Golden_Godsun制作v5.1.5) else (title [%Title%]--重启[%ReTimes%]次--内存=[%MinRAM%/%MaxRAM%]--上次重启于[%date:~5,2%.%date:~8,2%-%time:~0,2%:%time:~3,2%:%time:~6,2%]--Golden_Godsun制作v5.1.5)) else (if "%AutoPort%"=="开" (title [%Title%]--端口=[%Port%]--内存=[%MinRAM%/%MaxRAM%]--Golden_Godsun制作v5.1.5) else (title [%Title%]--内存=[%MinRAM%/%MaxRAM%]--Golden_Godsun制作v5.1.5))
...
echo #Golden_Godsun production, software version v5.1.5>>eula.txt
...
if exist "%~dp0modules.yml" (goto :B&V)
if exist "%~dp0velocity.toml" (goto :B&V)
...
if "%AutoBackup%"=="开" (goto :Backup) else (goto :stop)
:njava
if exist "%~dp0modules.yml" (goto :JavaB&V)
if exist "%~dp0velocity.toml" (goto :JavaB&V)
"%Java%" %JVM% -Xms%MinRAM% -Xmx%MaxRAM% -XX:+UseG1GC -XX:+ParallelRefProcEnabled -XX:MaxGCPauseMillis=200 -XX:+UnlockExperimentalVMOptions -XX:+DisableExplicitGC -XX:+AlwaysPreTouch -XX:G1NewSizePercent=30 -XX:G1MaxNewSizePercent=40 -XX:G1HeapRegionSize=8M -XX:G1ReservePercent=20 -XX:G1HeapWastePercent=5 -XX:G1MixedGCCountTarget=4 -XX:InitiatingHeapOccupancyPercent=15 -XX:G1MixedGCLiveThresholdPercent=90 -XX:G1RSetUpdatingPauseTimePercent=5 -XX:SurvivorRatio=32 -XX:+PerfDisableSharedMem -XX:MaxTenuringThreshold=1 -Dusing.aikars.flags=https://mcflags.emc.gs -Daikars.new.flags=true -jar %Core% nogui
if "%AutoBackup%"=="开" (goto :Backup) else (goto :stop)
:JavaB&V
"%Java%" %JVM% -Xms%MinRAM% -Xmx%MaxRAM% -XX:+UseG1GC -XX:G1HeapRegionSize=4M -XX:+UnlockExperimentalVMOptions -XX:+ParallelRefProcEnabled -XX:+AlwaysPreTouch -jar %Core% nogui
...
@echo.
@echo -----------------------------------------------------------------------------------
"%AppData%\Server Components\7-Zip\7z.exe" a -t7z "%BackupWay%\%BackupName%-[%date:~0,4%.%date:~5,2%.%date:~8,2%]-[%time:~0,2%.%time:~3,2%.%time:~6,2%].7z" "%~dp0*" -r -mf -mmt -mhc -mhcf -mx1 -ms=200m
title 服务器备份已完成，正在重启...
@echo -----------------------------------------------------------------------------------
@echo.
@echo                          服务器备份已完成，正在重启...
