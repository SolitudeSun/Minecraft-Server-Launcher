...
if "%LaSeconds%"=="开" (
    if "%AutoRestart%"=="开" (
        if "%AutoPort%"=="开" (
            title [%Title%]--重启[%ReTimes%]次--端口=[%Port%]-最近重启时间[%Ttime%]--内存=[%MinRAM%/%MaxRAM%]--Golden_Godsun制作v5.2.1) else (
            title [%Title%]--重启[%ReTimes%]次--最近重启时间[%Ttime%]--内存=[%MinRAM%/%MaxRAM%]--Golden_Godsun制作v5.2.1)) else (
        if "%AutoPort%"=="开" (
            title [%Title%]--端口=[%Port%]--内存=[%MinRAM%/%MaxRAM%]--Golden_Godsun制作v5.2.1) else (
            title [%Title%]--内存=[%MinRAM%/%MaxRAM%]--Golden_Godsun制作v5.2.1))) else (
    if "%AutoRestart%"=="开" (
        if "%AutoPort%"=="开" (
            title [%Title%]--重启[%ReTimes%]次--端口=[%Port%]--内存=[%MinRAM%/%MaxRAM%]--Golden_Godsun制作v5.2.1) else (
            title [%Title%]--重启[%ReTimes%]次--内存=[%MinRAM%/%MaxRAM%]--Golden_Godsun制作v5.2.1)) else (
        if "%AutoPort%"=="开" (
            title [%Title%]--端口=[%Port%]--内存=[%MinRAM%/%MaxRAM%]--Golden_Godsun制作v5.2.1) else (
            title [%Title%]--内存=[%MinRAM%/%MaxRAM%]--Golden_Godsun制作v5.2.1)))
:: 检测并同意EULA
...
@echo.
@echo -----------------------------------------------------------------------------------
mshta vbscript:msgbox("提示：未检测到'设置.ini'文件，已自动创建新的配置文件，请前往'设置.ini'设置启动参数！",vbSystemModal,"服务器启动软件v5.2.1     Golden_Godsun制作")(window.close)
goto :config
:2
...
@echo.
@echo -----------------------------------------------------------------------------------
mshta vbscript:msgbox("警告：设置文件版本错误，无法正常运行，已自动创建新的配置文件，请前往重设启动参数！",vbSystemModal,"服务器启动软件v5.2.1     Golden_Godsun制作")(window.close)
del /q 设置.ini >nul
goto :config
:3
...
@echo.
@echo -----------------------------------------------------------------------------------
mshta vbscript:msgbox("警告：自动检测设置参数错误，请检查是否为开或关！",vbSystemModal,"服务器启动软件v5.2.1     Golden_Godsun制作")(window.close)
goto :stop
:4
...
@echo.
@echo -----------------------------------------------------------------------------------
mshta vbscript:msgbox("提示：未检测到开服核心jar，请将核心复制到此软件所在目录！",vbSystemModal,"服务器启动软件v5.2.1     Golden_Godsun制作")(window.close)
goto :stop
:5
...
@echo.
@echo -----------------------------------------------------------------------------------
mshta vbscript:msgbox("警告：'%Core%'核心名称错误，有空格的名称请在'设置.ini'中进行设置！",vbSystemModal,"服务器启动软件v5.2.1     Golden_Godsun制作")(window.close)
goto :stop
:6
...
@echo.
@echo -----------------------------------------------------------------------------------
mshta vbscript:msgbox("警告：未检测到%Java%路径下的Java，请在'设置.ini'中重新设置Java路径！",vbSystemModal,"服务器启动软件v5.2.1     Golden_Godsun制作")(window.close)
goto :stop
:7
...
@echo.
@echo -----------------------------------------------------------------------------------
mshta vbscript:msgbox("提示：设置的重启次数限制已达到%ReLimits%次，已自动关闭服务器！",vbSystemModal,"服务器启动软件v5.2.1     Golden_Godsun制作")(window.close)
goto :stop
:8
...
@echo.
@echo -----------------------------------------------------------------------------------
mshta vbscript:msgbox("警告：您开启了自动备份，但未设置自动备份的路径，请前往'设置.ini'中设置路径！",vbSystemModal,"服务器启动软件v5.2.1     Golden_Godsun制作")(window.close)
goto :stop
:9
...
@echo.
@echo -----------------------------------------------------------------------------------
mshta vbscript:msgbox("警告：您设置的备份等级有错误，不在1-9的范围中，请前往'设置.ini'中设置路径！",vbSystemModal,"服务器启动软件v5.2.1     Golden_Godsun制作")(window.close)
goto :stop
:10
...
@echo.
@echo -----------------------------------------------------------------------------------
mshta vbscript:msgbox("警告：检测到服务器启动异常，已自动停止服务器！请检查启动参数，Java版本，Java配置环境，或者咨询其他人！",vbSystemModal,"服务器启动软件v5.2.1     Golden_Godsun制作")(window.close)
goto :stop
:: 停止信息及退出
:stop
@echo.
